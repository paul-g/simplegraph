...
public class MainFrame extends JFrame {
	...
	private JMenuItem nou, saveTab, saveAllTabs, closeSelTab, exit, defAccuracy,
			defIncAccuracy, supFunctions, about;
	JTabbedPane jtb;
	...
	private static void setLookAndFeel(String theme) throws ClassNotFoundException,
	...
	MainFrame() {
		...
		nou.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_N,
				ActionEvent.ALT_MASK));
		exit.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_X,
				ActionEvent.ALT_MASK));
		saveTab.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S,
				ActionEvent.CTRL_MASK));
		saveAllTabs.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S,
				ActionEvent.CTRL_MASK));
		saveAllTabs.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_A,
				ActionEvent.CTRL_MASK));
		closeSelTab.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_C,
				ActionEvent.CTRL_MASK));
		...
		String[] functionForm = { "+ - * / ()", "base^exponent", "sqrt()",
				"abs", "[],floor", "ln()", "sin() ", "cos()", "tg(x),tan(x)",
				"arcsin()", "arccos()", "arctg(),arctan()" };

		String[] functionDescription = {
				" Plus, minus, multiplication, division, grouping symbol. Multiplication can be omitted.",
				"Exponentiation baseexponent", "Square root of argument",
				"Absolute value of argument", "Rounds arg down",
				"Natural logarithm of argument", "Sine", "Cosine", "Tangent",
		...
		try {
			for (int i = 0; i < functionForm.length; i++) {
				...
				doc.insertString(doc.getLength(), functionForm[i] + newline,
						doc.getStyle("bold"));
				doc2.insertString(doc2.getLength(), functionDescription[i]
						+ newline, doc2.getStyle("regular"));
